const {
    ...
    multer = require("multer"), { imageKit } = require("../config");
...
module.exports = {
    create: async(req, res, next) => {
        try {
            upload(req, res, async(err) => {
                ...
                let { name, city, province, country, location, details, doctorId } =
                req.body;
    ...
    getMe: async(req, res, next) => {
        try {
            ...
            const get = await hospitalOnDoctor.findMany({
                where: {
                    doctorId: token.id,
                },
    ...
    destroyAllHospitalDoctor: async(req, res, next) => {
        try {
            ...
            res.json({
                success: "All relation hospital on doctor and related data deleted successfully",
            });
        } catch (error) {
            console.log(error);
            next(error);
